...
export function Sidebar({ user }: { user: User }) {
  ...
  return (
    <>
      ...
      {open && (
        <div onClick={() => setOpen(false)} className="lg:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-30" aria-hidden />
      )}
      ...
      >
        <div className="px-5 py-5 border-b border-white/10 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setOpen(false)} aria-label="Close menu" className="lg:hidden text-zinc-400 hover:text-white">
              ...
              return (
                <li key={item.href}>
                  <Link href={item.href} onClick={() => setOpen(false)}
                    ...
                    <span className={active ? "text-[#c9a56a]" : "text-zinc-500"}>{item.icon}</span>
                    {item.label}
                  </Link>
                </li>
              );
        ...
        <div className="border-t border-white/10 px-4 py-4">
          ...
          <div className="text-[10px] uppercase tracking-wider text-zinc-500 mt-0.5">{user.role ?? "admin"}</div>
          <button
            type="button"
            onClick={async () => {
